...
* `oriented_same_length_spectrum_rigidity`: Lean-checked conditional on the
  ...
  triple repeat of length `≥ L - 1`) and the case-split context
  `hcase : IsPrimitive ∨ ∃ p, HasMinimalPeriod`. Primitive and periodic
  truths are both covered; no global multiplicity-`≤ 2` hypothesis is imposed.
...
and primitive/periodic case-split context, every positive balanced circulation
of total mass `G` on the truth's oriented length-`L` window support is the
truth's ordinary length-`L` spectrum. -/
theorem oriented_same_length_spectrum_rigidity {α : Type} [DecidableEq α]
    ...
    (hcase : RepeatAdapter.IsPrimitive hG S ∨
      ∃ p : ℕ, RepeatAdapter.HasMinimalPeriod hG S p)
    (B : (Fin L → α) → ℕ)
  ...
  rcases hcase with hprim | ⟨p, hmin⟩
...
theorem spectrum_objective_tie {α M : Type} [DecidableEq α]
  ...
  have heq := oriented_same_length_spectrum_rigidity hG S hL hLG hno hcase
    B hBsup hBbal hBtot
...
theorem rotation_uniqueness_of_bbt {α : Type} [DecidableEq α]
    ...
    (hBtot : ∑ w ∈ OrientedRigidity.support hG S, B w = G)
    (T : Fin G → α)
    (hreal : ∀ w, B w = OrientedRigidity.specCount hG T w)
    (hBBT : ∀ T : Fin G → α,
      (∀ w, OrientedRigidity.specCount hG T w =
