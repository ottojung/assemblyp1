...
\emph{any} genome length, read length, or alphabet. Moreover, since $\Is$ supplies
Ukkonen's condition at $K=L-1$, the same complete-spectrum uniqueness input used
in the population model shows that every such candidate is a cyclic rotation of
the truth, so on this truth-admissible row the truth is the unique maximizer up to
rotation (Corollary~\ref{cor:same-length-rotation}). The equal-length restriction
