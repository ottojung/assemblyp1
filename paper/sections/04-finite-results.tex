...
Shomorony's $\Is$; the same complete-spectrum uniqueness input used by the
population model upgrades its conclusion from a tie to uniqueness up to cyclic
rotation. Then a sequence of \emph{negative} witnesses, each refuting a
...
Only the triple-repeat clause of $\Is$ is used for the spectrum-rigidity and
no-strict-counterexample conclusions; the interleaved-repeat and coverage clauses
are not needed there. The next corollary does use the interleaved clause, because
it invokes the complete-spectrum uniqueness input of
Section~\ref{sec:population}, whose hypothesis is Ukkonen's full condition.

\begin{corollary}[Uniqueness up to rotation on the truth-admissible same-length row]
\label{cor:same-length-rotation}
Under the hypotheses of Corollary~\ref{cor:same-length-zero}, every same-length
Section~6.2 spelled candidate $D$ is a cyclic rotation of $S$. Consequently the
truth is not merely a maximizer on this row: it is the unique maximizer up to
cyclic rotation.
\end{corollary}

\begin{proof}
Since $R\in\Is$, every copy of a triple repeat is bridged and every interleaved
maximal-repeat pair is bridged; a length-$L$ read bridges a copy of a repeat of
length $\ell$ only if $\ell\le L-2$. Hence $S$ satisfies Ukkonen's condition at
$K=L-1$. By Corollary~\ref{cor:same-length-zero},
$\spec_L(D)=\spec_L(S)$, so $D$ is an Eulerian circuit of the same $K$-mer graph
built from that complete $L$-spectrum. Theorem~\ref{thm:BBT} at $K=L-1$ gives a
unique Eulerian cycle for this graph, spelling $S$ up to cyclic rotation; hence
$D$ is a cyclic rotation of $S$. Since every candidate in the class has
likelihood ratio $1$ by Corollary~\ref{cor:same-length-zero}, the maximizer set is
exactly the rotation class of $S$.
\end{proof}

\statusline{The spectrum-rigidity reduction is \kernelcheck{} in
\texttt{AssemblyP1/OrientedFinalRigidity.lean}, conditional on the explicit
no-long-triple-repeat boundary, with the primitive/periodic split discharged
internally by the kernel-checked dichotomy; the
$\Is$-to-no-triple-repeat bridging implication is source-supported, and the
spectrum-identifiability (rotation-uniqueness) step is the external \srcfact{}
input \cite{bresler2013} (Theorem~\ref{thm:BBT}), the same input accepted for the
population theorem. The whole chain is source-supported mathematics with a
kernel-checked conditional core, not a kernel-checked proof of $\Is$ or BBT,
and it does not transfer to molecule classes.}

The equal length assumption, by contrast, is essential and sharp; we return to it
in Section~\ref{sec:finite:length}.
...
Corollary~\ref{cor:same-length-zero} are \mathfact{} with a \kernelcheck{}
conditional core (\texttt{OrientedFinalRigidity}, modulo the source-supported
$\Is$ bridging implication, the primitive/periodic split now being
kernel-checked internally); the rotation
conclusion of Corollary~\ref{cor:same-length-rotation} adds the external BBT input
\cite{bresler2013} and is therefore source-supported mathematics rather than
kernel-checked. The same-spectrum conclusion gives equality/ties for
spectrum-based finite objectives, not strict recovery by itself. The minimum non-rigid pair and the bounded verifications are
\computed, exhaustive in the cited scope.}
...
\midrule
same-length oriented Section~6.2, $R\in\Is$, truth §6.2-admissible
  & exact / binomial
  & same-length spelled
  & \textbf{true}, unique up to rotation (Thm.~\ref{thm:rigidity}, Cor.~\ref{cor:same-length-rotation}) \\
exact ML maximizer
  & exact multinomial
  & all circular
...
is recorded in the corresponding theorem or status line. The positive
same-length row asserts uniqueness up to cyclic rotation, not merely that the
truth ties. By Lemma~\ref{lem:negative-transfer} each same-length falsity
transfers to all circular candidates for that objective.}
\label{tab:finite}
\end{table}
...
some false, and at least one open. On the single positive row, the truth is the
unique maximizer up to cyclic rotation, so that row settles the uniqueness schema
as well as the maximizer schema; the update from a bare tie to uniqueness is the
finite counterpart of the population result in Section~\ref{sec:population}. The
negative results also expose the
