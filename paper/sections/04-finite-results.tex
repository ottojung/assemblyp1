...
\statusline{The rigidity step is \mathfact; the spectrum-identifiability step is
the external \srcfact{} input \cite{bresler2013} (Theorem~\ref{thm:BBT}), the same
input accepted for the population theorem. The whole chain is source-supported
mathematics, not a kernel-checked Lean theorem, and it does not transfer to
molecule classes.}
...
Corollary~\ref{cor:same-length-zero} are \mathfact; the rotation conclusion of
Corollary~\ref{cor:same-length-rotation} adds the external BBT input
\cite{bresler2013} and is therefore source-supported mathematics rather than
kernel-checked. The minimum non-rigid pair and the bounded verifications are
...
product with external $N$ (Definition~\ref{def:likelihoods}). The mechanism is different from the sequence-level
...
\[
  \frac{\Lik_{6.1}(D;x)}{\Lik_{6.1}(S;x)} = \frac{9}{8} > 1 .
\]
\end{theorem}
