...
\emph{kernel-checked} (Lean-checked finite certificate),
...
Same-length oriented Section~6.2 rigidity under $\Is$
  & source-supported mathematics with kernel-checked conditional core &
  Theorem~\ref{thm:rigidity}; reduction in \texttt{OrientedFinalRigidity} \\
No strict same-length Section~6.2 counterexample, any $G,L,\Alphabet$
  & source-supported mathematics with kernel-checked conditional core &
  Corollary~\ref{cor:same-length-zero} \\
...
Primitivity plus P2 gives gcd-one spectra (division + Eulerian + BBT),
  ...
  & project mathematical reduction (sketch) & repository synthesis \\
Population uniqueness, oriented primitive P2
  & source-supported mathematical theorem & Thm.~\ref{thm:population},
  \cite{bresler2013} \\
...
\bottomrule
\end{tabular}
\caption{Mathematical and project-level results. ``Project mathematical
reduction (sketch)'' marks the one step whose full writeup is not reproduced
here; it is not kernel-checked.}
\end{table}
...
Conditional same-length oriented spectrum rigidity (no-long-triple-repeat
boundary + primitive/periodic case split; $\Is$ implication and BBT external)
  & kernel-checked (conditional) & \texttt{OrientedFinalRigidity} \\
...
variable-length Section~6.2 statement; (9) a full kernel-checked proof of the
gcd-one/division step (currently a project sketch);
and (10) the population model for molecule read types.
