...
\subsection{Primitive P2 spectra: normalized to ordinary}
...
\label{def:P1P2}
Fix the read length $L$. An oriented circular word $D$ with $L\le|D|$ satisfies
...
$\spec_L(W)=c'$ and $|W|=|S|/g$ (windows of $W$ read cyclically, which is
also meaningful when $|W|<L$).
Repeating $W$ gives $W^g$ with $|W^g|=|S|$ and
$\spec_L(W^g)=g\cdot\spec_L(W)=c=\spec_L(S)$: each cyclic length-$L$
window of $W$ lifts to exactly $g$ windows of $W^g$ by $|W|$-periodicity,
including when $|W|<L$.
...
primitivity is rotation-invariant (a rotation of a $|W|$-periodic word is
again $|W|$-periodic) --- a contradiction. So $g=1$.
...
Let $S,D$ be primitive P2-admissible with $L\le|S|$ and $L\le|D|$, and
suppose $p_D=p_S$. Then $\spec_L(D)=\spec_L(S)$.
\end{corollary}

\begin{proof}[Proof sketch]
$p_D=p_S$ first gives equal supports, then rewrites as
$|S|\,\spec_L(D)=|D|\,\spec_L(S)$ on that support, so the two
...
equal (indeed $|S|\mid|D|$ and $|D|\mid|S|$ by B\'ezout, so $|S|=|D|$).
\end{proof}
...
Let $L\ge2$ and let the truth $S$ be primitive P2-admissible with
$L\le|S|$. Among primitive P2-admissible oriented circular candidates
$D$ with $L\le|D|$, the true genome $S$ is the unique population
maximum-likelihood genome up to cyclic rotation.
\end{theorem}

\begin{proof}
Let $D$ be primitive and P2-admissible with $L\le|D|$ and
$\ellpop_S(D)=\ellpop_S(S)$. By
...
gcd-one/division writeup is the project's.}
