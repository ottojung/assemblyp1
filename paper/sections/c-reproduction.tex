...
\begin{itemize}[leftmargin=*]
  ...
  \item same-length oriented rigidity:
        ...
        kernel-checked abstract circulation core and circular-spectrum adapter in
        \texttt{AssemblyP1/OrientedRigidity.lean}, the primitive/periodic
        repeat-theory adapter in \texttt{AssemblyP1/RepeatAdapter.lean}, and the
        thin final conditional spectrum-rigidity reduction in
        \texttt{AssemblyP1/OrientedFinalRigidity.lean}.  The reduction is
        kernel-checked conditional on the explicit no-long-triple-repeat boundary,
        with the primitive/periodic split discharged internally by the
        kernel-checked dichotomy; the $\Is$ bridging implication
        and the rotation-uniqueness (BBT) input remain external;
