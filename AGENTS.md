...
- Treat the shared Lubko pool as a research resource that should stay **close to 5 useful active agents** whenever the frontier supports that much non-overlapping work. Scheduled runs should take a lightweight live snapshot near startup and launch useful non-overlapping agents into clearly free capacity, preferably in small batches. New AssemblyP1 agents must have titles beginning `AssemblyP1:` so the live pool is machine-identifiable. Five is a soft utilization target and a hard maximum, not a synchronization barrier or quota. Agent-management failures or uncertain recounts must not block useful mathematical research; in that case stop launching uncertain additional agents and continue work that is safe under the cap.
