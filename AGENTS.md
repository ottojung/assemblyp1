...
- Treat the shared Lubko pool as a research resource that should stay **close to 5 useful active agents** whenever the frontier supports that much non-overlapping work. Before launching, reconcile repository-wide live worker state; fill only free slots, and refill useful slots promptly as workers finish or are stopped. Five is a target saturation level, not a quota for low-value or duplicate work. Agents draw from the whole research frontier, not just currently open GitHub issues: issue-less exploratory packets are allowed, and durable discoveries should be promoted into issues/docs/PRs when they become worth preserving. The orchestrator may work directly on tiny deterministic tasks, inherently serial coordination, and final reconciliation/review.
