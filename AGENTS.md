...
- Treat the shared Lubko pool as a research resource that should stay **close to 5 useful active agents** whenever the frontier supports that much non-overlapping work. Every scheduled run must perform the repository's mandatory Lubko saturation preflight before substantive local work: query live managed agents on `phoebe-dev`, count AssemblyP1 workers, and fill useful free slots. New AssemblyP1 agents must have titles beginning `AssemblyP1:` so the live pool is machine-identifiable. Five is a target saturation level, not a quota for low-value or duplicate work. Agents draw from the whole research frontier, not just currently open GitHub issues. The orchestrator may work directly on tiny deterministic tasks, inherently serial coordination, and final reconciliation/review.
