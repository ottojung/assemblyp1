# The smallest MB09 §6.2 bridging, flow-feasible truth beaten by an admissible flow

_Status: source reading + derivation + exact computation, 2026-09-20, for issue
#36. The finding is that the current independent-model note
([`independent-se62-bidirected-model.md`](independent-se62-bidirected-model.md))
searched only **distinct** read starts (`n ≤ G`) and therefore reported `G = 5`
as the smallest witness; allowing the source's **sampling with replacement**
(`n > G`) moves the smallest witness down to `G = 4`._
...
1. **A bridging, flow-feasible truth is beaten by another admissible flow.**
   The smallest witness is at `G = 4`:
...
2. **The determination is complete, not a bounded search.** For this
   ...
   `d*`, at any `o_min` and for any feasible set. [derivation]

3. **Smallest is `G = 4`, and it needs `n > G`.** `G = 3` has no witness
   (proved, §4). `G = 4` has no witness with distinct starts (`n ≤ G`); the
   first witnesses appear at `n = 5` and `n = 6` for the single molecule class
   `{AAAT, ATTT}`. [derivation + computation]

4. **The `G = 5` witnesses remain valid but are not smallest.** Under the same
   model `AAATT` (ratio `9/8`) and `AATAT` (ratio `243/128`) are witnesses with
   distinct starts. Their best admissible flows are themselves spellable.
   [computation]
...
- **Reduction.** At `o_min = L−1 = 2` no length-2 edge can be transitive over
  ...
  the Myers-vs-literal reduction convention. At `o_min = 1` the length-1 edges
  matter; the witness is verified directly there as well (its edges have length
  `2`).
...
## 3. The smallest witness, checked end to end
...
**`I_s` holds (strict predicate).** [computation, from the script]
...
§6.1 optimum over all admissible flows. [derivation]
...
**`G = 3` has no witness (proof).** [derivation] The binary length-3 molecule
classes are `{AAA}`, `{AAT, ATT}`, `{TTT}`. For the homopolymer classes the
only observed type is the truth type and the likelihood is maximised at the
...
`∏_w 2^{2x_w − n} = 2^{−n} < 1`. So no flow ever beats the truth. The script
also checks every start multiset with `n ≤ 8` exhaustively.

**`G = 4` with distinct starts has no witness.** Exhaustive over all rotation
classes, all `n ≤ G`, and all distinct start sets. [computation]

**`G = 4` with `n > G` has witnesses, first at `n = 5`.** Exhaustive over all
start multisets with `n ≤ G + 2`. [computation]

Therefore the smallest §6.2 witness is `G = 4`, and it requires sampling **with
replacement** (`n > G`).
...
| Artifact | Relation |
|---|---|
| [`independent-se62-bidirected-model.md`](independent-se62-bidirected-model.md) (this branch) | Its bounded neighbourhood search restricted to **distinct** starts (`n ≤ 4`), so it reported `G = 5`. This note **extends** it: allowing repeated starts lowers the smallest witness to `G = 4`. Its `AAATT`/`AATAT` witnesses and `9/8`/`243/128` ratios are reproduced. |
| [`section62-aaatt-reduced-flow-cone-and-spectra.md`](section62-aaatt-reduced-flow-cone-and-spectra.md) | The independent derivation of the exact `AAATT` cone (`d_AAT = d_TAA`) and the exact optimum `(2,2,2)` are consistent; here the same "integer target" argument also closes the `G = 4` case. |
| unmerged `analysis/issue36-nonspellable-*` branches (`section62-flow-beats-bridged-truth.md`, `section62-nonspellable-flow-counterexample-broadened.md`) | They exhibit a **non-spellable** flow beating `AATAT` at `o_min = 1` on the **unreduced** folded relation. Under the maximal-overlap (`o_min = 2`) transitively reduced graph that flow `(AAT:1, ATA:2, TAA:1)` is **not feasible** (checked in the script), and the best flow for `AATAT` is the spellable `(2,2,2)` at `243/128`. So the "smallest non-spellable" claim is a modeling-convention statement, not a source-faithful one at `o_min = L−1`; it does not lower the smallest source-faithful witness below `G = 4`. |

**Convention note.** The unmerged branch's own table already records
`representative-reduced, o_min = 2` as having zero non-spellable beats; this note
is consistent with that row and isolates the `o_min`/reduction convention as the
entire source of the difference.

---

## 6. What this does and does not settle

**Does.** It exhibits a source-faithful bridging, flow-feasible truth (`AAAT`)
whose admissible §6.2 flow is beaten by another admissible flow, with an exact
global optimum argument, and it pins the smallest such witness at `G = 4`
(requiring `n > G`). This strengthens the negative answer to the §6.2-restricted
implication from a bounded search to a complete determination for that witness.
...
question; nor does it settle the source-faithful transitive reduction at
`o_min = 1` beyond the directly verified witness. The result is a
...
| `d* = (2,1,1,1)` is the global §6.1 optimum over all admissible flows for that instance | **derivation** (+ computation cross-check) |
| `G = 3` has no witness; `G = 4` needs `n > G`; smallest is `G = 4` | **derivation + computation** |
