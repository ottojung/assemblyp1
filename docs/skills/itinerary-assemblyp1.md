...
Lubko's canonical skill is the operational authority for reaching `lubko://marceline-dev`: follow its Supabase job transport, polling/liveness, and execution-safety rules rather than improvising a direct shell connection. AssemblyP1's managed-agent CLI is provided separately on Marceline by the pinned pre-removal Lubko checkout documented in `docs/skills/lean-on-marceline-dev.md`; do not assume current Lubko `main` still contains `lubko-agent`. `marceline-dev` has Guix available and is the preferred execution host for both delegated research and Lean evaluators. AssemblyP1-specific rules in this itinerary still govern research selection, the repository-wide five-agent cap, and direct integration into `main`.
...
Before running Lean or managed agents on `marceline-dev`, follow [`lean-on-marceline-dev.md`](lean-on-marceline-dev.md). Marceline uses the pinned wrapper-backed `lubko-agent` runtime and may obtain Lean through Guix or another suitable project setup; do not depend on Phoebe-specific Elan paths.
