...
AssemblyP1 has a hard **repository-wide** concurrency limit: **no more than 5 delegated agents may be actively working in parallel across all concurrent orchestrators**. This overrides generic Lubko guidance that would otherwise allow more. Use a lightweight live snapshot to avoid obvious overfill, launch only into clearly free capacity, and keep pool bookkeeping subordinate to research. New AssemblyP1 agents must use an `AssemblyP1:` title prefix.
