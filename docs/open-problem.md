...
A primary-source trace of what the phrase “the maximum-likelihood formulation of the AP (Medvedev and Brudno, 2009)” does and does not denote is recorded in [`docs/source-notes/shomorony-mb-formulation-provenance.md`](source-notes/shomorony-mb-formulation-provenance.md); it separates the source facts from the reading and keeps the candidate-universe choice explicit. A focused follow-up, [`docs/source-notes/shomorony-ml-quantifier-sequence-resolution.md`](source-notes/shomorony-ml-quantifier-sequence-resolution.md), uses a version-edit argument (the accepted sentence replaces an earlier explicit “GHC of a desired length `G`” objective) to resolve the *object type*: the open question quantifies over single candidate sequences, not MB09 §6.2 flows, while the probability model, length convention, and tie semantics remain open. An independent reconciliation of the four candidate referents, the witness-sufficiency question, and the existing notes is [`docs/source-notes/mb-formulation-referent-reconciliation.md`](source-notes/mb-formulation-referent-reconciliation.md). A decision on the remaining question, whether MB09 §6.2 flow feasibility is genuinely necessary to settle the published question, is [`docs/source-notes/se62-feasibility-necessity-determination.md`](source-notes/se62-feasibility-necessity-determination.md): it finds that §6.2 is MB09’s algorithm rather than the sequence-level referent, and that the §6.2-restricted sequence-level implication is already refuted by a kernel-checked witness, so the genuinely necessary residual choices are the probability model and the length/tie conventions. A third residual choice is the sample-size quantifier: [`docs/source-notes/issue36-finite-vs-asymptotic-regime.md`](source-notes/issue36-finite-vs-asymptotic-regime.md) shows the sentence is phrased per read set, that the kernel-checked witnesses settle only that per-instance reading, and that under fixed-length high coverage the truth is asymptotically the unique maximizer (conditional on the circular spectrum-uniqueness bridge). The accepted text selects none of the four referents; the repository must keep the referent, the candidate class, the tie semantics, and the regime explicit rather than silently choosing.
