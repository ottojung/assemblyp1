...
For the current positive oriented theorem, `¬HasLongTripleRepeat` is a restricted sufficient hypothesis for the existing rigidity chain, not a consequence of source-faithful `I_s`. The kernel-checked circular truth `AAAAB` at `G = 5`, `L = 3`, with all starts observed satisfies the full source-faithful information-feasibility condition while still having a long maximal triple repeat. Thus the old proposed bridge `I_s → ¬HasLongTripleRepeat` is false. The remaining oriented same-length ML frontier is the wraparound regime: either prove ML domination there directly or exhibit a truth-feasible counterexample in the applicable literal §6.2 candidate class. Likewise, upgrading equality of complete spectra to genome uniqueness up to rotation uses the external Bresler–Bresler–Tse complete-spectrum theorem unless and until that theorem is separately formalized.
