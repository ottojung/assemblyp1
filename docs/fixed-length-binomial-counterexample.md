...
This note originally did **not** show that `D` is feasible for the Section 6.2 transitively reduced bidirected-overlap-graph flow problem. That hedge is now resolved in the direction of feasibility under the loop-inclusive reading: `docs/flow-feasibility-aaacc-witness.md` exhibits an explicit Section 6.2 circulation realizing `D` (the `AAA` self-overlap is the load-bearing loop) and shows that `D` beats the truth under both the Section 6.2 vertex-cost objective (`9/8`) and the full Section 6.1 product of binomials (`1125/512`). Section 6.2 still has its own read-derived feasible set and may represent non-contiguous assemblies; `docs/source-notes/medvedev-brudno-candidate-class.md` records why it must not be silently identified with the Section 6.1 circular-candidate universe.
